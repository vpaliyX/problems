...
V rbbstree<K,V>::min() {
  auto result = _min();
  return result != nullptr ? result->value : V{};
}
...
template<class K, class V>
V rbbstree<K,V>::find(const K &key) {
  auto target = _find(key);
  return target != nullptr ? target->value : V{};
}
...
template<class K, class V>
void rbbstree<K,V>::transplant(node<K,V>* first, node<K,V>* second) {
...
template<class K, class V>
void rbbstree<K,V>::remove(const K &key) {
  auto target = _find(key);
  if (target != nullptr) {
    auto is_black = target->red;
